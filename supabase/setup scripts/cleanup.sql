...
-- created by the master_setup.sql script.
-- Date: 2025-05-05
-- =========================================================
...
BEGIN
  IF EXISTS (SELECT FROM information_schema.schemata WHERE schema_name = 'storage') THEN
    ...
    -- Drop policies on storage.objects first (specific policies created by master setup)
    RAISE NOTICE 'Dropping storage policies...';
    DROP POLICY IF EXISTS "Allow authenticated users manage own lead uploads" ON storage.objects;
    DROP POLICY IF EXISTS "Allow service_role full access to lead uploads" ON storage.objects;
    -- Add drops for other bucket policies if created ('templates', 'generated-documents')
...
BEGIN
  ...
  IF EXISTS (
      ...
      -- AND tgrelid = 'auth.users'::regclass -- More robust check
  ) THEN
      DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
      RAISE NOTICE 'Dropped trigger on_auth_user_created on auth.users.';
  ELSE
      RAISE NOTICE 'Trigger on_auth_user_created on auth.users not found.';
  END IF;
...
DROP FUNCTION IF EXISTS public.archive_normalized_leads(uuid, text) CASCADE; -- Updated signature
DROP FUNCTION IF EXISTS public.normalize_archive_table_types(text) CASCADE;
-- DROP FUNCTION IF EXISTS public.get_latest_lead_filename() CASCADE; -- (Commented out in master_setup)
-- Drop other functions if they were present in the old setup_functions.sql and not consolidated
...
DROP FUNCTION IF EXISTS public.fn_trigger_normalize_leads() CASCADE;

RAISE NOTICE 'Finished dropping custom functions.';

-- Step 5: Drop all dynamic/archive lead tables (matching pattern from archive function)
DO $$
DECLARE
  tbl_name text;
BEGIN
  RAISE NOTICE 'Dropping dynamic/archive lead tables...';
  FOR tbl_name IN
    SELECT table_name
    FROM information_schema.tables
    WHERE table_schema = 'public'
    AND table_name LIKE 'archive_leads_%' -- Match pattern from archive_normalized_leads function
    AND table_name NOT IN ('leads', 'campaign_leads', 'normalized_leads') -- Exclude base tables
  LOOP
    EXECUTE format('DROP TABLE IF EXISTS public.%I CASCADE', tbl_name);
    RAISE NOTICE 'Dropped dynamic/archive table %', tbl_name;
  END LOOP;
  RAISE NOTICE 'Finished dropping dynamic/archive lead tables.';
END
$$;

-- Step 6: Drop all standard tables in correct dependency order
-- Drop indexes explicitly first (before dropping tables)
...
-- Add indexes related to new tables/columns from master_setup
DROP INDEX IF EXISTS idx_lead_sources_user_id;
DROP INDEX IF EXISTS idx_leads_user_id;
DROP INDEX IF EXISTS idx_leads_lead_source_id;
DROP INDEX IF EXISTS idx_email_templates_user_id;
DROP INDEX IF EXISTS idx_document_templates_user_id;
DROP INDEX IF EXISTS idx_senders_user_id;
DROP INDEX IF EXISTS idx_processing_status_user_id;
DROP INDEX IF EXISTS idx_processing_status_job_type;
DROP INDEX IF EXISTS idx_processing_status_status;
DROP INDEX IF EXISTS idx_campaign_leads_user_id;
DROP INDEX IF EXISTS idx_campaign_senders_user_id;
-- DROP INDEX IF EXISTS idx_profiles_id; -- Usually handled by PK
...
-- Drop main tables (add new tables here)
DROP TABLE IF EXISTS public.processing_status CASCADE; -- Added
...
DROP TABLE IF EXISTS public.senders CASCADE; -- Added
DROP TABLE IF EXISTS public.profiles CASCADE;

RAISE NOTICE 'Finished dropping standard tables.';
...
BEGIN
    ...
    FROM information_schema.tables
    WHERE table_schema = 'public'
    AND table_name LIKE 'archive_leads_%' -- More specific pattern from archive function
    AND table_name NOT IN ('leads', 'campaign_leads', 'normalized_leads');
